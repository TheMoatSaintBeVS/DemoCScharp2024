...
{
    public class UnitTestReposMemesController
    {
        ...
        //[Fact]
        //public void TestGetAllMemesNullNull()
        //{
        //    var memesController = new ReposMemesController(new Memes.Repositories.InMemory.Repository());
        ...
        public void TestGetAllMemesNullCountWithMOQ(int currentValue)
        {
            ...
            ((List<Demo.WebAPI.MemesTags.ViewModels.MemeVM>)result.Value).Should().HaveCount(limit, $"Because we added ${limit} MEMES");
        }
        [Fact]
        public void TestGetOneMeme()
        {
            ...
            mockRepository.Setup(x => x.GetAllMemes()).Returns(_memes);
            mockRepository.Setup(x => x.FindMeme(It.IsAny<int>())).Returns<int>( i=> _memes.Find(m => m.Id ==i));
        ...
        public void TestPostMemeOK()
        {
            ...
            mockRepository.Setup(self => self.FindImage(It.IsAny<int>())).Returns<int>(i => _images.Find(y=> y.Id == i));

            mockRepository.Setup(self => self.FindUser(It.IsAny<int>())).Returns<int>( i => _users.Find(y => y.Id == i));

            mockRepository.Setup(x => x.FindTag(It.IsAny<int>())).Returns<int>( i=> _tags.Find(y => y.Id == i));

            mockRepository.Setup(x => x.Add(It.IsAny<Meme>())).Returns<Meme>( m => 1 );
