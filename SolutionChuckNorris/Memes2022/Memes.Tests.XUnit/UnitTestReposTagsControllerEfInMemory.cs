...
namespace Memes.Tests.XUnit
{
    ...
    public class DatabaseFixture : IDisposable
    {
        ...
        private  MemesDbContext _dbMemesContext;
        public MemesDbContext MemesDbContext => _dbMemesContext;
        ...
        private readonly List<Image> _images
           = new List<Image>
           {
               new Image { Id  = 10 , Uri= "https://64.media.tumblr.com/c84a55767d03c0e8a37d90b415ad05a9/tumblr_pfawdltFE21qgszpno1_640.jpg"},
               new Image { Id  = 20, Uri = "https://64.media.tumblr.com/0e419d8b6bb164d66bf53b834f6438cd/tumblr_p8ibt9zc2T1qgszpno1_400.png"},
               new Image { Id  = 30, Uri = "https://64.media.tumblr.com/d90c2a597988ec50a24f22c7add3dd2c/tumblr_oxyyisPOHF1qgszpno4_540.png"}
           };
        ...
        public void Init()
        {
            ...
            _dbMemesContext.AddRange(_users.Where(u => !_memes.Any(m => m.User.Id == u.Id)).Select(x => x).ToList());

            _dbMemesContext.AddRange(_images.Where(i => !_memes.Any(m => m.Image.Id == i.Id)).Select(x => x).ToList());

            // only the tags that are not already created with 
            // the memetags 

            var newTags = _tags.Where(t => !_memes.SelectMany(m => m.MemeTags).Any(mt => mt.TagId == t.Id)).Select(x => x).ToList();
            _dbMemesContext.AddRange(newTags);

            _dbMemesContext.AddRange(_memes);




            _dbMemesContext.SaveChanges();
        ...
        {
            if (_dbMemesContext == null)
            {
                var builder = new DbContextOptionsBuilder<Memes.Repositories.EfCore.MemesDbContext>();
                builder.UseInMemoryDatabase("DbMemesInMemory");

                _dbMemesContext = new Memes.Repositories.EfCore.MemesDbContext(builder.Options);
                Init();
            }
    ...
    public class UnitTestReposTagsControllerEfInMemory : IClassFixture<DatabaseFixture>
    {

        DatabaseFixture _databaseFixture;
        public UnitTestReposTagsControllerEfInMemory(DatabaseFixture fixture)
        {
        ...
        public void TestGetAllMemesNullCountWithMOQ(int currentValue)
        {
            ...
            var efReposInMemory = new TagRepository(_databaseFixture.MemesDbContext);
            ...
            var reposTagsController = new ReposTagsController(efReposInMemory)
            {
